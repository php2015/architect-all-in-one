...
const resolvePromise = (promise2, x, resolve, reject) => {
  ...
  if ((typeof x === "object" && x !== null) || typeof x === "function") {
    try {
      // 这个部分捕获异常主要是因为 then 方法可能是人为使用defineProperty 定义的 取指的时候可能会发生异常
...
class Promise {
  ...
  static all(promises) {
    return new Promise((resolve, reject) => {
      ...
      let times = 0
      // 处理成功的逻辑
      const precessSuccess = (index, val) => {
        result[index] = val
      ...
      for (let i = 0; i < promises.length; i++) {
        ...
        if (p && typeof p.then === "function") {
          p.then((data) => {
            precessSuccess(i, data)
          }, reject) // 这里面直接将外层的reject函数传递进来了 某一个promise失败了 直接执行失败即可
        } else {
          precessSuccess(i, p) // 普通值直接放进去
        }
      }
    })
  }
